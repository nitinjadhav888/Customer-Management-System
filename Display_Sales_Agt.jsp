    ...
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js">
        ...
        <script>
            function getConfirmation()
            {
               ...
               if(res==true)
               {
                  window.location.href="DisplaySales_Agt.jsp"; 
               }
               else
               {
                   return false;
        ...
        <div class="container-fluid">
            <form>
                   <h3 class="text-center">Sales Details</h3>
                    <div class="row">
                        <div class="col-sm-4">
                    <a href="SalesForm_Agt.jsp" class="btn btn-info">
                        <span class="glyphicon glyphicon-plus"></span>  Add New Sales
                    </a>
                        </div>
...
<% 
                            ...
                            if(x_btn!=null)
                            {
                                String fetch_record= request.getParameter("search_req_data");
                                 sql ="select * from sales_entry_agent where sales_id = '"+fetch_record+"' or fname='"+fetch_record+"' or lname='"+fetch_record+"'or phone_no='"+fetch_record+"'or email='"+fetch_record+"'or pan_card='"+fetch_record+"'or aadhar_card='"+fetch_record+"'or cash_amount='"+fetch_record+"' or loan_amount='"+fetch_record+"' or entry_by='"+fetch_record+"'or date='"+fetch_record+"'";
                            ...
                            {
                                 sql ="select * from sales_entry_agent order by sales_id desc";
                            }
%>
           ...
           <tr>
              <td><%= x %></td>
                        ...
                        <td><%= rs.getString("AADHAR_CARD") %></td>
                         <td><%= rs.getString("CASH_AMOUNT") %></td>
                        <td><%= rs.getString("LOAN_AMOUNT") %></td>
                        <td><%= rs.getString("DATE") %></td>
                        <td><%=rs.getString("ENTRY_BY")%></td>
               <td class="text-center"><a href="Delete_Sales_Entry_Agt.jsp?id=<%=rs.getInt("sales_id")%>" class="btn btn-danger" onclick="return getConfirmation()">
                       <span class="glyphicon glyphicon-trash"></span> Delete
                   </a>
               </td>
               <td><a href="Update_Sales_Agt.jsp?id=<%=rs.getInt("sales_id")%>" class="btn btn-success">
